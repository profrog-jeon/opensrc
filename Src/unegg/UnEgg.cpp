...
void PrintUsage(void)
{
	...
	printf("-P[PWD]\tSet a password for extract.\n");
...
int PrintListOfArchive(const char* filename)
{
    ...
    if (res == Result::Success)
    {
		...
		strSplitter.resize(g_console.ws_col);
        memset((void*)strSplitter.c_str(), 0, g_console.ws_col);
        memset((void*)strSplitter.c_str(), '-', g_console.ws_col - 1);
        ...
        if (g_verbose > 0)
        {
            ...
            if (g_verbose < 2)
            {
                ...
                {
                    if (g_disp & ((int) pow(2.0, (int)i)))
                    {
						core::PRINTF2("%c%%%ds", isFirst ? ' ' : g_delimiter, HEADERS[i].length, HEADERS[i].title);
                ...
                if (g_disp & 1)
                {
					core::PRINTF("%c%%s", isFirst ? ' ' : g_delimiter, HEADERS[0].title);
        ...
        for (size_t i = 0; i < info->AsSize(key::archive::FILE_COUNT); ++i)
        {
            ...
            if (g_verbose < 2)
            {
                ...
                {
                    if (g_disp & (int) pow(2.0, (int)j))
                    {
                        char fmt[20] = { 0, };
						core::SafeSPrintf(fmt, sizeof(fmt), "%c%%%d%s", isFirst ? ' ' : g_delimiter, HEADERS[j].length, HEADERS[j].type);
                ...
                if (g_disp & 1)
                {
					core::PRINTF("%c%%s", isFirst ? ' ' : g_delimiter, utf8::path_ellipsis(restOfLine, finfo->AsString(key::file::name::NAME)));
                }
            }
            else
            {
                ...
                {
                    char fmt[20];
					core::SafeSPrintf(fmt, sizeof(fmt), "\t%%11s : %%%s\n", HEADERS[j].type);
...
int main(int argc, char* argv[])
{
    ...
    if (cmd.GetArgumentCount() < 1 || cmd.GetCommandCount() > 2)
    {
        PrintProgramDetails();
