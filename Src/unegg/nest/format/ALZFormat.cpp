                    ...
                    while (remained > 6)
                    {
                        ...
                        if (read == 6)
                        {
                            ...
                            if (read == len)
                            {
                                ...
                                if (index == 0xFFFFFFFF)
                                {
									//std::string strUTF8 = UTF8FromEUCKR(comment);
                                }
                                else if (files_.size() > index)
                                {
                                    std::string strUTF8 = core::UTF8FromMBS(comment);
                                    files_[index]->SetComment(strUTF8.c_str());
                                }
                                else
                                {
                                    res = Result::Invalid;
...
int ALZFormat::GenerateVolumeName(size_t index, char* buffer, size_t size)
{
    ...
    core::SafeSPrintf(buffer, size, "%s.%s", name.c_str(), extension);
        ...
        while (qc < 4)
        {
            ...
            else
            {
				if( g_strPassword.empty() )
					...
					core::SafeStrCpy(password, alz::MAX_PASSWORD_LEN, g_strPassword.c_str());
            }
    ...
    else
    {
        ...
        if (res == Result::Success)
        {
            if (file->GetBitFlags())
            {
                ...
                if (info.coder)
                {
					std::vector<BYTE> vecDataHeader;
					if (info.dataHeaderSize > 0)
					{
						vecDataHeader.resize(info.dataHeaderSize);
						istream_.Read(&vecDataHeader[0], info.dataHeaderSize);
						res = info.coder->Initialize(&vecDataHeader[0], info.dataHeaderSize);
					}
					else
						res = info.coder->Initialize(NULL, 0);
